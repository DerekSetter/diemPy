...
class GenotypeData:
    ...
    def get_mask_array(self, biallelic=False):
        """
        Only included SNPs        Non SNPs are not written to excluded
        """
...
def get_exclusions(encoded_df, limit=None, exclude_missing_homs=None):
    ...
    df.replace([i for i in [str(x) for x in range(3,10)]], 'U', inplace=True)

    unc_count_arr = df.eq('U').sum(axis=1)
   
   
    #with pd.option_context("future.no_silent_downcasting", True):
    #    df = df.replace("_", np.nan)
   
    row_sum_arr = 2 * df.eq('2').sum(axis=1) + 1 * df.eq('1').sum(axis=1)
